...
checkins_dropout['Lat'] = np.radians(checkins_dropout['Lat']) # for sklearn distance only
checkins_dropout['Long'] = np.radians(checkins_dropout['Long']) # for sklearn distance only
...
#++++++++++++++++++++++++++++++++++++++++++++ dynamic bandwith geographical similarity ++++++++++++++++++++
...
dist_sort_k_nearest=np.delete(dist_sort, np.s_[k_nearest:n_venues], axis=1) 
print('dist_sort_k_nearest')
print(dist_sort_k_nearest.shape)
print(dist_sort_k_nearest)
print("-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.")

dist_avg=np.mean(dist_sort_k_nearest, axis=1)  
print(dist_avg)
print("-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.")
...
print("-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.")
#h=(np.max(dist_mat)-np.min(dist_mat))*2/100 
print("Max Dist: ", np.max(dist_mat))
print("Max Dist: ", np.min(dist_mat))

kernel_type='Gaussian distance-decay-based weighting'

proximity=np.exp(-dist_mat**2/(2*hp**2))
del dist_mat
del hp
#proximity=1-(dist_mat-np.min(dist_mat))/(np.max(dist_mat)-np.min(dist_mat))
print(proximity)
...
user_venues_matrix=user_venues_matrix[np.where(np.count_nonzero(user_venues_matrix, axis=1)>=16)]
print('new user_venues based on users that visit more than 16 venues:') 
...
for user in range(user_venues_matrix.shape[0]):
    test_size =int(len(user_venues_matrix[user, :].nonzero()[0])*15/100)
    #print(test_size)
